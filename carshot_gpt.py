...
def call_gpt(image_buf: io.BytesIO, prompt: str) -> bytes:
    """
    Schritt 1: GPT-4o Vision analysiert das Auto (Farbe, Marke, Modell, Details)
    Schritt 2: DALL-E 3 generiert das Auto im Showroom basierend auf der Beschreibung
    """
    import base64 as b64lib

    # Bild als base64 fuer Vision
    image_buf.seek(0)
    ...
    vision_response = client.chat.completions.create(
        ...
        messages=[{{
            ...
            "content": [
                ...
                {{
                    "type": "text",
                    "text": """Describe this car in extreme detail for a photo recreation:
- Exact make, model, year
- Exact paint color and finish (metallic, matte, glossy)
- All visible body details, trim, badges, wheels
- Camera angle and perspective
- Any unique features
Be very specific and detailed. Only describe the car, not the background."""
                }}
            ]
        }}],
        max_tokens=500
    ...
    dalle_prompt = f"""Professional automotive dealership showroom photograph.

Car: {{car_description}}

Setting: Premium automotive showroom with polished light grey concrete floor with subtle wet reflections, floor-to-ceiling glass windows showing softly blurred green trees outside, soft overhead LED spotlights, warm accent lighting from the sides, minimalist modern white interior walls. Realistic floor reflection of the car.

Style: Professional automotive press photo, photorealistic, 8K quality. No people, no other vehicles, no text."""
